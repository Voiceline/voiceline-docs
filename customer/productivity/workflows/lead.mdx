---
title: "Lead"
description: "Capture a new prospect from the field in under a minute — by voice or business card scan — with automatic CRM deduplication before creation."
status: Live
bundles:
  - customer_data
...
For business card scanning, you can also tap the camera icon directly from the home screen — see [Business Card Scanner](/customer/productivity/assistant#adding-context).
...
Yes — business card scanning is optimized for high-volume trade fair scenarios. See [Industry Fair Mode](/customer/productivity/assistant#fair-mode) for the full fair workflow.
...
<Note>Lead fields, source values, deduplication logic, and CRM mapping are configured during your VoiceLine implementation. See [Lead Standard Implementation](/implementation/standard-implementations/lead) for the default configuration, or [Product Customisation](/implementation/product-customisation) for what can be adjusted.</Note>
