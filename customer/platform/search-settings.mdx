...
<Note>**Favourites** are set by individual reps directly in the app — no admin action required.</Note>
...
Flexible CRM lookups (CRMFlex) always search by both name and city — reps can type a city name to find records located there. Name matches are ranked higher than city matches.
